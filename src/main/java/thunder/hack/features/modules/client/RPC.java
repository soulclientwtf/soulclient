...
public final class RPC extends Module {
        ...
        if (!started) {
            ...
            thread = new Thread(() -> {
                while (!Thread.currentThread().isInterrupted()) {
                    ...
                    presence.button_url_1 = "https://github.com/shawtysystem/phasmo-client";

                    switch (mode.getValue()) {
                        case Recode -> presence.largeImageKey = "https://imgur.com/a/P3AM6aP";
                        case MegaCute ->
    ...
    private String getDetails() {
        ...
        if (mc.currentScreen instanceof MultiplayerScreen || mc.currentScreen instanceof AddServerScreen || mc.currentScreen instanceof TitleScreen) {
            ...
            result = isRu() ? (showIP.getValue() ? "Сосёт на " + mc.getCurrentServerEntry().address : "Сосёт на сервере") : (showIP.getValue() ? "Sucking on " + mc.getCurrentServerEntry().address : "Sucking on server");
        } else if (mc.isInSingleplayer()) {
            result = isRu() ? "Сосёт в одиночке" : "Sucking in single";
        }
        return result;
    }
