...
public class Velocity extends Module {
            ...
            if (pac.getId() == mc.player.getId() && (!onlyAura.getValue() || ModuleManager.aura.isEnabled())) {
                switch (mode.getValue()) {
                    case GrimFreeze -> {
                        ...
                        ((ISPacketEntityVelocity) pac).setMotionX(0);
                        ((ISPacketEntityVelocity) pac).setMotionY(0);
                        ((ISPacketEntityVelocity) pac).setMotionZ(0);
                        // Disabler: отправляем фейковые пакеты чтобы сбить Grim с толку
                        sendPacket(new PlayerMoveC2SPacket.OnGroundOnly(true));
                        sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, BlockPos.ofFloored(mc.player.getPos()), Direction.UP));
                    }
                    case Matrix -> {
                        if (!flag) {
                            e.cancel();
        ...
        switch (mode.getValue()) {
            case GrimFreeze -> {
                if (mc.player.hurtTime > 0) {
                    mc.player.setVelocity(0, 0, 0);
                    // Продолжаем отправлять фейковые пакеты во время hurtTime
                    if (mc.player.age % 3 == 0) {
                        sendPacket(new PlayerMoveC2SPacket.OnGroundOnly(mc.player.isOnGround()));
                        sendPacket(new PlayerActionC2SPacket(PlayerActionC2SPacket.Action.START_DESTROY_BLOCK, BlockPos.ofFloored(mc.player.getPos()), Direction.UP));
                    }
                }
            }
    ...
    public enum modeEn {
        Matrix, Cancel, Sunrise, Custom, Redirect, OldGrim, Jump, GrimNew, GrimFreeze
    }
