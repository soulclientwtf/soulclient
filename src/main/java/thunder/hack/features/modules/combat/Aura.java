...
public class Aura extends Module {
    ...
    private boolean haveWeapon() {
        ...
        if (onlyWeapon.getValue()) {
            // Проверяем, что в руке есть валидное оружие
            ...
            if (switchMode.getValue() == Switch.None) {
                ...
                // Если есть валидное оружие в руке, разрешаем атаку
                // Иначе проверяем наличие в хотбаре
    ...
    private void calcRotations(boolean ready) {
        ...
        // Увеличиваем плавность для Grim
        ...
        // Добавляем дополнительный джиттер для Grim
    ...
    public float getSquaredRotateDistance() {
        ...
        if (rotationMode.getValue() != Mode.Ares || rayTrace.getValue() == RayTrace.OFF)
            dst = getRange();
    ...
    public Vec3d getLegitLook(Entity target) {
        ...
        if (!mc.player.canSee(target)) {
            ...
            if (rotationMode.getValue() == Mode.Ares) {
                ...
                int point = (int) MathUtility.random(0, 2);
                double yOffset;

                switch (point) {
                    case 0 -> yOffset = height * 0.85;
                    case 1 -> yOffset = height * 0.5;
                    case 2 -> yOffset = height * 0.15;
                    default -> yOffset = height * 0.5;
                }

                double xOffset = (Math.random() - 0.5) * width * 0.2;
                double zOffset = (Math.random() - 0.5) * width * 0.2;
    ...
    public enum Mode {
        Interact, Ares, TrackNew, Grim, None
    }
